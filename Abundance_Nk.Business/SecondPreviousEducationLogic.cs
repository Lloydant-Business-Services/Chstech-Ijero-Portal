...
{
    public class SecondPreviousEducationLogic : BusinessBaseLogic<SecondPreviousEducation, APPLICANT_SECOND_PREVIOUS_EDUCATION>
    {
        public SecondPreviousEducationLogic()
        {
            translator = new SecondPreviousEducationTranslator();
        }

        public SecondPreviousEducation GetBy(ApplicationForm applicationForm)
        {
            try
            {
        ...
        public bool Modify(SecondPreviousEducation previousEducation)
        {
            try
            {
                Expression<Func<APPLICANT_SECOND_PREVIOUS_EDUCATION, bool>> selector = p => p.Applicant_Second_Previous_Education_Id == previousEducation.Id;
                APPLICANT_SECOND_PREVIOUS_EDUCATION entity = GetEntityBy(selector);
